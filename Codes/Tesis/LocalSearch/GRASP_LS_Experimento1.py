...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 23 16:36:23 2023
...
localsearch = 0
elapsedtimeStop = 3600 # 40 minutos
modelStopTime = 180 # 3 minutos 
alpha_def = 0.30
maxIterGRASP = 100 #Deben ser 100 aprox
            ...
            for verif in rates:
                ...
                if TotalAccidentes != 0:
                    ...
                    while sumaelapsed < elapsedtimeStop and iteracionGRASP < maxIterGRASP:
                        ...
                        colnames = ["name", "I size", "L size", "S size", "resp time", "0", "time", "elapsed time", "best obj", "best bound", "gap %", "status", "LS", "iteracion GRASP", "iteracion LS"]
                        for column in range(len(colnames)):
                            sheet.write(0, column, colnames[column])
                        name = str('Instance')+str('_')+str(len(I))+str('_')+str(len(L))+str('_')+str(len(S))+ '_' + str(eta[0]) + "_" + str(eta[1])+ '_' + str(verif) + '_'  
                        sheet.write(countcsv, 0, name)
                        sheet.write(countcsv, 1, len(I))
                        sheet.write(countcsv, 2, len(L))
                        sheet.write(countcsv, 3, len(S))
                        sheet.write(countcsv, 4, verif)
                        sheet.write(countcsv, 5, 0)
                        if len(model._data) != 0:
                            print(" Entra datos for ")
                            datos = model._data[len(model._data)-1]
                            for row in range(len(datos)):
                                sheet.write(countcsv, row+6, datos[row])
                        else:
                            sheet.write(countcsv, 6, time.time()-model._start)
                            sheet.write(countcsv, 7, model._sumaelapsed)
                            sheet.write(countcsv, 8, model.objVal)
                            sheet.write(countcsv, 9, model.objVal)
                        sheet.write(countcsv, 13, iteracionGRASP)
                        countcsv = countcsv + 1
                    
                        with open('data_DemandFixed_'+str(len(I))+str('_')
                                          +str(len(L))+str('_')
                                          +str(len(S))+ '_' + str(eta[0]) + "_" + str(eta[1])+ '_' + str(verif) + '_'  +'.csv', 'w') as fi:
                                writer = csv.writer(fi)
                                writer.writerows(model._data)
                        
                        #Nombre: Resultados_Prueba_I_L_M_N_S
                        
                        f = open ('Resultados_Prueba_DemandFixed_'
                                      +str(len(I))+str('_')
                                      +str(len(L))+str('_')
                                      +str(len(S))+ '_' + str(eta[0]) + "_" + str(eta[1])+ '_' + str(verif) + '_'  +'.txt','w')
                        ...
                        coberturas = open ('Coberturas_DemandFixed_'
                                      +str(len(I))+str('_')
                                      +str(len(L))+str('_')
                                      +str(len(S))+ '_' + str(eta[0]) + "_" + str(eta[1])+ '_' + str(verif) + '_'  +'.txt','w')                      
                        
                        lectura = open ('Resultados_Prueba_DemandFixed_'
                                      +str(len(I))+str('_')
                                      +str(len(L))+str('_')
                                      +str(len(S))+ '_' + str(eta[0]) + "_" + str(eta[1])+ '_' + str(verif) + '_'  +'.txt','r')
                        lectura.readline()
                        lectura.readline()
                        ...
                        model.write('model_DemandFixed_'+str(len(I))+str('_')
                                      +str(len(L))+str('_')
                        ...
                        if model.objVal > valorObjetivo:
                            ...
                            mejoras = open ("mejoras_DemandFixed_"
                                      +str(len(I))+str('_')
                                      +str(len(L))+str('_')
                                      +str(len(S))+ '_' + str(eta[0]) + "_" + str(eta[1])+ '_' + str(verif) + '_'  +'.txt','w')
                            ...
                            coberturas = open ('Coberturas_DemandFixed_'
                                          +str(len(I))+str('_')
                                          +str(len(L))+str('_')
                                          +str(len(S))+ '_' + str(eta[0]) + "_" + str(eta[1])+ '_' + str(verif) + '_'  +'.txt','a')                      
                        ...
                        while sumaelapsed < elapsedtimeStop and iteracionLS < cantIteracionesTotales:    
                                ...
                                for j in range(len(L)):
                                    ...
                                    if j in potentialSiteActivos and j != initialL and prob_aux > probVerificacion: #LS3
                                        ...
                                        if aux + aux1 > 1:    
                                            ...
                                            with open('data_DemandFixed_'+str(len(I))+str('_')
                                                          +str(len(L))+str('_')
                                                          +str(len(S))+ '_' + str(eta[0]) + "_" + str(eta[1])+ '_' + str(verif) + '_'  +'.csv', 'a') as f: #Cambiar de w a a
                                                writer = csv.writer(f)
                                            ...
                                            if model.objVal > valorObjetivo:
                                                ...
                                                if len(model._data) != 0:
                                                    ...
                                                    sheet.write(countcsv, 8, model.objVal)
                                                    sheet.write(countcsv, 9, model.objVal)
                                                sheet.write(countcsv, 12, "LS2")
                                                sheet.write(countcsv, 13, iteracionGRASP)
                                                sheet.write(countcsv, 14, iteracionLS)
                                                countcsv = countcsv + 1
                                                ...
                                                mejoras.write(str(initialSolution))
                                                mejoras.write("\n")
                                                print("entra else que 0ite solution")
                                                print(" ")
                                                print(" ")
                                                #soluciones.append(vecino)
                                            ...
                                            if maxLSinterno > cantIteracionesTotales:
                                                break
