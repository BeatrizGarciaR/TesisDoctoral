# -*- coding: utf-8 -*-
"""
Created on Wed Mar  9 19:55:58 2022
    ...
    for jconj in range(len(tamaños_L)):
        for sconj in range(len(tamaños_S)):
            ...
            for s in range(len(S)):
                for l in L:
                    for i in I:
                        ycopia_vars[s+1,l,2,i] = model.addVar(vtype=GRB.BINARY, 
                                                 name="dispatched copia "+str(s+1)+str(' ')+str(l)+str(' ')+str(2)+str(' ')+str(i))
              
            z_vars = {}
            for s in range(len(S)):
                for i in I:
                    for k in K:
                        z_vars[s+1,i,k] = model.addVar(lb=0.0, ub=0.5, vtype=GRB.CONTINUOUS, 
                                                  name="porcentage "+str(s+1)+str(' ')+str(i)+str(' ')+str(k))
            ...
            for s in range(len(S)):
                for i in I:
                    if S[s][i-1][0]!=0:
                        obj += z_vars[s+1,i,1]*gp.quicksum(cli[l-1][i-1]*y_vars[s+1,l,k,i] for l in L for k in K)/(len(S)*(S[s][i-1][0]))
                    
                    if S[s][i-1][1]!=0:
                        obj += z_vars[s+1,i,2]*gp.quicksum(cli[l-1][i-1]*ycopia_vars[s+1,l,2,i] for l in L)/(len(S)*(S[s][i-1][1]))
                    
                    obj -= p*gamma_vars[s+1,i]/len(S)
                ...
                for l in L:
                    for k in K:
                        if k == 1:
                            ...
                            model.addConstr(gp.quicksum(y_vars[s+1,l,k,i] + ycopia_vars[s+1,l,k,i] for i in I) <= x_vars[l,k], "c3_1")
                
                for i in I:
                    if S[s][i-1][0] != 0:
                        model.addConstr(z_vars[s+1,i,1] == gp.quicksum(y_vars[s+1,l,k,i] for k in K for l in L)/(len(K)*S[s][i-1][0]), "c4")
                
                for i in I:
                    if S[s][i-1][1] != 0:
                        model.addConstr(z_vars[s+1,i,2] == gp.quicksum(ycopia_vars[s+1,l,2,i] for l in L)/(len(K)*S[s][i-1][1]), "c4_1")
                
                for i in I:
                    if S[s][i-1][0] != 0:
                        model.addConstr(gp.quicksum(y_vars[s+1,l,k,i] + ycopia_vars[s+1,l,2,i] for k in K for l in L) + gamma_vars[s+1,i] >= 1, "c5")
                
                #Auxiliar constraints
                for i in I:
                    for k in K:
                        if S[s][i-1][0] != 0:
                            model.addConstr(gp.quicksum(y_vars[s+1,l,k,i] for k in K for l in L)/S[s][i-1][0] <= 1, "c6")
                
                for i in I:
                    for k in K:
                        if S[s][i-1][1] != 0:
                            model.addConstr(gp.quicksum(ycopia_vars[s+1,l,2,i] for l in L)/S[s][i-1][1] <= 1, "c6_1")
                            
                for l in L:
                    for i in I:
                        model.addConstr(y_vars[s+1,l,2,i] + ycopia_vars[s+1,l,2,i] <= 1, "c7")
                        
                for i in I:
                    model.addConstr(z_vars[s+1,i,1] + z_vars[s+1,i,2] <= 1, "c8")
            ...
            for s in range(len(S)):
                ...
                for i in I: 
                    dispatches[s].append([])
                    for k in K: 
                        dispatches[s][i-1].append(dispatches_original[s][i-1][k-1] + dispatches_copia[s][i-1][k-1])
            #print(dispatches)
            #print()
