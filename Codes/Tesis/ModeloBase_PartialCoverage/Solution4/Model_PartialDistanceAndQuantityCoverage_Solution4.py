# -*- coding: utf-8 -*-
"""
Created on Mon Feb  7 20:54:17 2022
...
#         a_aux[s].append([])
#         for num in range(2):
#             #if S[s][i-1][num] == 0: #Todos los aik
#             if S[s][i-1][0] == 0 and S[s][i-1][1] == 0: #solo cuando i = 0
#                 a_aux[s][i-1].append(1000)
#             else:
#                 a_aux[s][i-1].append(S[s][i-1][num])
...
#                                            for l in L for k in K)/(a_aux[s][i-1][0] + a_aux[s][i-1][1]) 
...
for s in range(len(S)):
    ...
    #     print()
    #     if a[s][i-1][1] > 0:
    #         model.addConstr(gp.quicksum(y_vars[s+1,i,l,1] for l in L) == 0, "c4")
            
    for i in I:
        model.addConstr(gp.quicksum(y_vars[s+1,l,1,i] for l in L) <= S[s][i-1][0], "c4")
    
    for i in I:
        if S[s][i-1][1] == 0:
            model.addConstr(gp.quicksum(y_vars[s+1,l,k,i] for l in L for k in K) <= S[s][i-1][0], "c5")
...
f = open ('Resultados_Prueba_'
              ...
              +str(len(N))+str('_')
              +str(len(S))+str('_')
              +str('Solution4_aik=0_todos')+'_.txt','w')
...
model.write('model_'+str(len(I))+str('_')
              ...
              +str(len(K))+str('_')
              +str(len(N))+str('_')
              +str(len(S))+str('_')+str('Solution4_aik=0_todos')+'_.lp')
model.write('model_'+str(len(I))+str('_')
              +str(len(L))+str('_')
              +str(len(K))+str('_')
              +str(len(N))+str('_')
              +str(len(S))+str('_')+str('Solution4_aik=0_todos')+'_.mps')
...
resultados = open ('Resultados_Prueba_'
              ...
              +str(len(K))+str('_')
              +str(len(N))+str('_')
              +str(len(S))+str('_')+str('Solution4_aik=0_todos')+'_.txt','r')
...
fobj = open ('FObj_'
              ...
              +str(len(K))+str('_')
              +str(len(N))+str('_')
              +str(len(S))+str('_')+str('Solution4_aik=0_todos')+'_.txt','w')
