# -*- coding: utf-8 -*-
"""
Created on Wed Nov  8 17:21:18 2023
...
sheet = book.add_sheet('Tesis_ObjZs_Scenarios_281123', cell_overwrite_ok=True)
...
for iconj in range(len(tamaños_I)):
        ...
        for sconj in range(len(tamaños_S)):
            ...
            #for rep in range(repeticiones):
                ...
                for s in range(len(S)):
                    ...
                    # Restricción 8: Desactivar alpha (cobertura total)
                    suma_alpha2 = gp.LinExpr()
                    for i in I:
                        if S[s][i-1][0] + S[s][i-1][1] > 0:
                            if S[s][i-1][0] != 0:
                            ...
                            model.addConstr((S[s][i-1][0]+S[s][i-1][1])*alpha_vars[s+1,i] <= suma_alpha2, "c8")
                    ...
                    # Restricción 10: Desactivar beta (cobertura parcial 1)
                    suma_beta1 = gp.LinExpr()
                    suma_beta2 = gp.LinExpr()
                    for i in I:
                        if S[s][i-1][0] + S[s][i-1][1] > 0:
                            if S[s][i-1][0] != 0:
                                suma_beta1 += gp.quicksum(cli[l-1][i-1]*y_vars[s+1,l,1,i] + cli[l-1][i-1]*y_vars[s+1,l,2,i] for l in L)
                                suma_beta2 += gp.quicksum(y_vars[s+1,l,1,i] + y_vars[s+1,l,2,i] for l in L)
                            if S[s][i-1][1] != 0 and S[s][i-1][0] == 0:
                ...
                with open('data_ObjZs_Scenarios_281123_'+str(len(I))+str('_')
                              +str(len(L))+str('_')
                              #+str(len(K))+str('_')
                              #+str(len(N))+str('_')
                ...
                f = open ('Resultados_Prueba_ObjZs_Scenarios_281123_'
                              +str(len(I))+str('_')
                              +str(len(L))+str('_')
                              #+str(len(K))+str('_')
                              #+str(len(N))+str('_')
                ...
                # coberturas = open ('Coberturas_ObjZs_Scenarios_281123_'
                #               +str(len(I))+str('_')
                #               +str(len(L))+str('_')
                #               +str(len(S))+'_'+str(eta[0])+'_'+str(eta[1])+'.txt','w')                      
                
                # lectura = open ('Resultados_Prueba_ObjZs_Scenarios_281123_'
                #               +str(len(I))+str('_')
                #               +str(len(L))+str('_')
    ...
    #             feasible.close()
                ...
                book.save('Tesis_ObjZs_Scenarios_281123_'+str(eta[0])+'_'+str(eta[1])+'.xls') 
