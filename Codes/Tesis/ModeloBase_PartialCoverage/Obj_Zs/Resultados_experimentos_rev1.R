...
counti = 0
#for (i in len_I){
pdf(paste("Objective_",name1,"_",len_L[1],'_',eta[1],'_',eta[2],"_1.pdf", sep=""), width = 10)
par(mfrow = c(1, length(amb)/2), mar=c(4.5, 5, 3.1, 0.9))
for (a in 1:length(amb[,1])){
  # run time
  eta <- amb[a,]
  filas = c(seq(from=(1+counti*(5*length(len_I))), to=((1+counti*(5*length(len_I)))+((5*length(len_I))-1))))
  aux_0 <- as.data.frame(read.csv(paste('Tesis_',name1,'_',len_L[1],'_',eta[1],'_',eta[2],'.csv', sep="")))
...
counti = 0
#for (i in len_I){
pdf(paste("TimeVal_",name1,"_",len_L[1],'_',eta[1],'_',eta[2],"_1.pdf", sep=""), width = 10)
par(mfrow = c(1, length(amb)/2), mar=c(4.5, 5, 3.1, 0.9))
for (a in 1:length(amb[,1])){
  # run time
  eta <- amb[a,]
