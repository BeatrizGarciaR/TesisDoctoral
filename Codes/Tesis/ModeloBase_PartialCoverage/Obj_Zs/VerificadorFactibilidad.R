  ...
  for (l in len_L){
    for (s in len_S){
      ...
      for (a in 1:length(amb[,1])){
        ...
        k1_located = 0
        k2_located = 0
        located <- delayed <- as.data.frame(read.table(paste("Location_Obj_NewModel_Supuesto_160124_", i,"_", len_L[1],"_", s, "_", eta[1], "_", eta[2],".txt", sep="")))
        ...
        k1_located = 0
        k2_located = 0
        located <- delayed <- as.data.frame(read.table(paste("Location_Obj_NewModel_NewModel_160124_", i,"_", len_L[1],"_", s, "_", eta[1], "_", eta[2],".txt", sep="")))
        for (locate in 1:length(located[,1])){
          if(located[locate,2] == 1){
            k1_located = k1_located + located[locate,3]
          }
        ...
        # Verificar si se despacharon <= de las localizadas
        delayed <- as.data.frame(read.table(paste("Delayed_Obj_NewModel_NewModel_160124_", i,"_", len_L[1],"_", s, "_", eta[1], "_", eta[2],".txt", sep="")))
        onTime <- as.data.frame(read.table(paste("OnTime_Obj_NewModel_NewModel_160124_", i,"_", len_L[1],"_", s, "_", eta[1], "_", eta[2],".txt", sep="")))
...
write.csv(factibilidad, file = paste('feasibility_AllInstances_AllAmb','.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")