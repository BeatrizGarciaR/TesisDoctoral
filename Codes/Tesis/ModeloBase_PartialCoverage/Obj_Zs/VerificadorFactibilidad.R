  ...
  for (l in len_L){
    for (s in len_S){
      ...
      for (a in 1:length(amb[,1])){
        ...
        # k1_located = 0
        # k2_located = 0
        # located <- delayed <- as.data.frame(read.table(paste("Location_Obj_NewModel_Supuesto_300124_", i,"_", len_L[1],"_", s, "_", eta[1], "_", eta[2],".txt", sep="")))
        ...
        # delayed <- as.data.frame(read.table(paste("Delayed_Obj_NewModel_Supuesto_300124_", i,"_", len_L[1],"_", s, "_", eta[1], "_", eta[2],".txt", sep="")))
        # onTime <- as.data.frame(read.table(paste("OnTime_Obj_NewModel_Supuesto_300124_", i,"_", len_L[1],"_", s, "_", eta[1], "_", eta[2],".txt", sep="")))
        # 
        # k1_del_fact <- c()
        # k2_del_fact <- c()
        # k1_ont_fact <- c()
        ...
        instance <- c()
        instance <- cbind(instance, "M1SinSupuesto", i, l, s, eta[1], eta[2])
        
        # Verificar si se localizaron <= ambulancias disponibles en el sistema
        k1_located = 0
        k2_located = 0
        located <- delayed <- as.data.frame(read.table(paste("Location_Obj_NewModel_NewModel_300124_", i,"_", len_L[1],"_", s, "_", eta[1], "_", eta[2],".txt", sep="")))
        for (locate in 1:length(located[,1])){
          if(located[locate,2] == 1){
            k1_located = k1_located + located[locate,3]
          }
        ...
        # Verificar si se despacharon <= de las localizadas
        delayed <- as.data.frame(read.table(paste("Delayed_Obj_NewModel_NewModel_300124_", i,"_", len_L[1],"_", s, "_", eta[1], "_", eta[2],".txt", sep="")))
        onTime <- as.data.frame(read.table(paste("OnTime_Obj_NewModel_NewModel_300124_", i,"_", len_L[1],"_", s, "_", eta[1], "_", eta[2],".txt", sep="")))
...
write.csv(factibilidad, file = paste('feasibility_AllInstances_AllAmb_300124','.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")