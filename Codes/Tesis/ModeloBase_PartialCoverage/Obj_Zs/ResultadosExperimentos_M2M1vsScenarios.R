...
for (a in 1:length(amb[,1])){
  ...
  aux_1 <- as.data.frame(read.csv(paste('Tesis_',name_1,'_',len_L[1],'_',eta[1],'_',eta[2],'.csv', sep="")))
  ...
  legend(x = "left",         # Posici??n
         legend = c("best objective MEC(SABC)", "best objective MEC"), # Textos de la leyenda
...
for (a in 1:length(amb[,1])){
  ...
  aux_1 <- as.data.frame(read.csv(paste('Tesis_',name1_1,'_',len_L[1],'_',eta[1],'_',eta[2],'.csv', sep="")))
