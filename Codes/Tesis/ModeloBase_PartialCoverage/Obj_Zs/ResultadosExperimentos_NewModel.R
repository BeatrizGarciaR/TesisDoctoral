...
for (a in 1:length(amb[,1])){
  ...
  aux_0 <- as.data.frame(read.csv(paste('Tesis_NewModel_NewModel_160124_35_20.csv', sep="")))
  ...
  pdf(paste("Timeval_NewModel_NewModel_160124_",eta[1],"_",eta[2],".pdf", sep=""))
  plot(matrix[1:5], pch=15, col=1, cex=1.5, ylim=c(0, 3500),
...
for (a in 1:length(amb[,1])){
  ...
  for (i in len_I){
    for (l in len_L){
      for (s in len_S){
        ...
        print(paste('I_Accidents_NewModel_160124_', i,'_',l,'_',s,eta[1],'_',eta[2],'.txt'))

        accident_aux <- as.data.frame(read.table(paste('I_Accidents_NewModel_NewModel_160124_', i,'_',l,'_',s,'.txt', sep="")))
        accidents <- as.data.frame(read.table(paste('Accidents_NewModel_NewModel_160124_', i,'_',l,'_',s,'.txt', sep="")))


        aux_0 <- as.data.frame(read.table(paste('OnTime_Obj_NewModel_NewModel_160124_',i,'_',l,'_',s,'_',eta[1],'_',eta[2],'.txt', sep="")))
        aux_0 <- aux_0[,-2]
        colnames(aux_0) <- c("S", "L", "K", "I", "OnTime")


        aux_1 <- as.data.frame(read.table(paste('Delayed_Obj_NewModel_NewModel_160124_',i,'_',l,'_',s,'_',eta[1],'_',eta[2],'.txt', sep="")))
        aux_1 <- aux_1[,-2]
        colnames(aux_1) <- c("S", "L", "K", "I", "Delayed")


        aux_2 <- as.data.frame(read.table(paste('NotAssigned_Obj_NewModel_NewModel_160124_',i,'_',l,'_',s,'_',eta[1],'_',eta[2],'.txt', sep="")))
        aux_2 <- aux_2[,-2]
  ...
  write.csv(accidents_covered, file = paste('ExpectedCoverage_Obj_NewModel_NewModel_160124_', eta[1],'_',eta[2],'.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
...
write.csv(accidents_covered_total, file = paste('ExpectedCoverageTotal_Obj_NewModel_NewModel_160124_','.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
...
for (a in 1:length(amb[,1])){
  ...
  for (i in 1:length(len_I)){
    datos <- c()
    datos <- cbind(datos, len_I[i], 30, eta[1], eta[2])
...
write.csv(prom_coverage, file = paste('MeanCoverageTotal_Obj_NewModel_NewModel_160124_','.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")


for (a in 1:length(amb[,1])){
  eta = amb[a,]
  pdf(paste("Coverage_NewModel_NewModel_160124_",eta[1],"_",eta[2],".pdf", sep=""))
