...
pdf(paste("Coverage_NewModel_Supuesto_060224.pdf", sep=""), width = 19.5)
par(mfrow = c(1, 3))
for (a in 1:length(amb[,1])){
  eta = amb[a,]
  #pdf(paste("Coverage_NewModel_Supuesto_060224_",eta[1],"_",eta[2],".pdf", sep=""))
  if(a == 1){
    plot(as.numeric(prom_coverage[a*5-4, 5:9]), pch=15, col=1, cex=1.5, cex.lab=1.7, cex.axis = 3.5,
         ylim=c(0, 100), ylab="% accidents coverage", xlab = "Coverage type", xaxt = "n",
         main=paste(eta[1],"BLS and",eta[2],"ALS ambulances", sep=" "))
  } else{
    plot(as.numeric(prom_coverage[a*5-4, 5:9]), pch=15, col=1, cex=1.5, cex.lab=1.7, cex.axis = 3.5,
         ylim=c(0, 100), ylab="% accidents coverage", xlab = "Coverage type", xaxt = "n", yaxt="n",
         main=paste(eta[1],"BLS and",eta[2],"ALS ambulances", sep=" "))
  }
