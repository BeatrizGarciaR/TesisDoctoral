...
for (a in 1:length(amb[,1])){
  ...
  aux_0 <- as.data.frame(read.csv(paste('Tesis_NewModel_NewModel_051223_35_20.csv', sep="")))
...
for (a in 1:length(amb[,1])){
  ...
  for (i in len_I){
    for (l in len_L){
      for (s in len_S){
        ...
        print(paste('I_Accidents_NewModel_051223_', i,'_',l,'_',s,eta[1],'_',eta[2],'.txt'))

        accident_aux <- as.data.frame(read.table(paste('I_Accidents_NewModel_051223_', i,'_',l,'_',s,'.txt', sep="")))
        accidents <- as.data.frame(read.table(paste('Accidents_NewModel_051223_', i,'_',l,'_',s,'.txt', sep="")))


        aux_0 <- as.data.frame(read.table(paste('OnTime_Obj_NewModel_051223_',i,'_',l,'_',s,'_',eta[1],'_',eta[2],'.txt', sep="")))
        aux_0 <- aux_0[,-2]
        colnames(aux_0) <- c("S", "L", "K", "I", "OnTime")


        aux_1 <- as.data.frame(read.table(paste('Delayed_Obj_NewModel_051223_',i,'_',l,'_',s,'_',eta[1],'_',eta[2],'.txt', sep="")))
        aux_1 <- aux_1[,-2]
        colnames(aux_1) <- c("S", "L", "K", "I", "Delayed")


        aux_2 <- as.data.frame(read.table(paste('NotAssigned_Obj_NewModel_051223_',i,'_',l,'_',s,'_',eta[1],'_',eta[2],'.txt', sep="")))
        aux_2 <- aux_2[,-2]
  ...
  write.csv(accidents_covered, file = paste('ExpectedCoverage_Obj_NewModel_051223_', eta[1],'_',eta[2],'.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
...
write.csv(accidents_covered_total, file = paste('ExpectedCoverageTotal_Obj_NewModel_051223_','.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")


###### IR CAMBIANDO ESTO POR LOS L ############3
ind_1 = 1
ind_2 = 5
...
write.csv(prom_coverage, file = paste('MeanCoverageTotal_Obj_NewModel_051223_','.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")


for (a in 1:length(amb[,1])){
  eta = amb[a,]
  pdf(paste("Coverage_051223_",eta[1],"_",eta[2],".pdf", sep=""))
  plot(as.numeric(prom_coverage[a*5-4, 5:9]), pch=15, col=1, cex=1.5,
       ...
       main=paste("Mean coverage percentage for 30 potential sites \n considering",eta[1],
                  "BLS and",eta[2],"ALS ambulances", sep=" "))
