...
# counti = 0
# #for (i in len_I){
# pdf(paste("Objective_NewModel_Supuesto_280224_",len_L[1],'_',eta[1],'_',eta[2],".pdf", sep=""), width = 10)
...
#   aux_0 <- as.data.frame(read.csv(paste('Tesis_NewModel_Supuesto_280224_',len_L[1],'_',eta[1],'_',eta[2],'.csv', sep="")))
#   
...
#   }
#   if(a == 1){
#     plot(matrix[1:length(len_I)], pch=15, col=1, cex=1.5, ylim=c(min(matrix)-20,max(matrix)+500),
...
# counti = 0
# #for (i in len_I){
# pdf(paste("Objective_NewModel_Supuesto_280224_",len_L[1],'_',eta[1],'_',eta[2],"_1.pdf", sep=""), width = 10)
# par(mfrow = c(1, length(amb)/2), mar=c(4.5, 5, 3.1, 0.9))
# for (a in 1:length(amb[,1])){
#   # run time
#   eta <- amb[a,]
...
#   
#   if(a == 1){
#     plot(matrix_1[1:length(len_S)], pch=15, col=1, cex=1.5, ylim=c(min(matrix_1)-20,max(matrix_1)+500),
...
# counti = 0
# #for (i in len_I){
# pdf(paste("Timeval_NewModel_Supuesto_280224_",len_L[1],'_',eta[1],'_',eta[2],".pdf", sep=""), width = 10)
# par(mfrow = c(1, length(amb)/2), mar=c(4.5, 5, 3.1, 0.9))
# for (a in 1:length(amb[,1])){
#   # run time
#   eta <- amb[a,]
...
# counti = 0
# #for (i in len_I){
# pdf(paste("Timeval_NewModel_Supuesto_280224_",len_L[1],'_',eta[1],'_',eta[2],"_1.pdf", sep=""), width = 10)
# par(mfrow = c(1, length(amb)/2), mar=c(4.5, 5, 3.1, 0.9))
# for (a in 1:length(amb[,1])){
#   # run time
#   eta <- amb[a,]
...
# # counti = 0
# # #for (i in len_I){
# # ###pdf(paste("Timeval_NewModel_Supuesto_280224_",eta[1],"_",eta[2],".pdf", sep=""))
# # pdf(paste("Timeval_NewModel_Supuesto_280224.pdf", sep=""), width = 20)
# # par(mfrow = c(1, 3), mar=c(4.5, 5, 3.1, 0.9))
# # for (a in 1:length(amb[,1])){
# #   # run time
# #   eta <- amb[a,]
# #   filas = c(seq(from=(1+counti*25), to=((1+counti*25)+24)))
# #   aux_0 <- as.data.frame(read.csv(paste('Tesis_NewModel_Supuesto_280224_35_20.csv', sep="")))
...
# # #for (i in len_I){
# # ###pdf(paste("Timeval_NewModel_Supuesto_280224_",eta[1],"_",eta[2],".pdf", sep=""))
# # pdf(paste("Timeval_NewModel_Supuesto_280224_1.pdf", sep=""), width = 20)
# # par(mfrow = c(1, 3), mar=c(4.5, 5, 3.1, 0.9))
# # for (a in 1:length(amb[,1])){
# #   eta <- amb[a,]
# #   matrix_new <- t(matrix)
...
for (a in 1:length(amb[,1])){
  ...
  for (i in len_I){
    for (l in len_L){
      for (s in len_S){
        ...
        print(paste('I_AccidentsNewModel_Supuesto_280224_', i,'_',l,'_',s,eta[1],'_',eta[2],'.txt'))

        accident_aux <- as.data.frame(read.table(paste('I_Accidents_NewModel_Supuesto_280224_', i,'_',l,'_',s,'.txt', sep="")))
        accidents <- as.data.frame(read.table(paste('Accidents_NewModel_Supuesto_280224_', i,'_',l,'_',s,'.txt', sep="")))


        aux_0 <- as.data.frame(read.table(paste('OnTime_Obj_NewModel_Supuesto_280224_',i,'_',l,'_',s,'_',eta[1],'_',eta[2],'.txt', sep="")))
        aux_0 <- aux_0[,-2]
        colnames(aux_0) <- c("S", "L", "K", "I", "OnTime")


        aux_1 <- as.data.frame(read.table(paste('Delayed_Obj_NewModel_Supuesto_280224_',i,'_',l,'_',s,'_',eta[1],'_',eta[2],'.txt', sep="")))
        aux_1 <- aux_1[,-2]
        colnames(aux_1) <- c("S", "L", "K", "I", "Delayed")


        aux_2 <- as.data.frame(read.table(paste('NotAssigned_Obj_NewModel_Supuesto_280224_',i,'_',l,'_',s,'_',eta[1],'_',eta[2],'.txt', sep="")))
        aux_2 <- aux_2[,-2]
  ...
  write.csv(accidents_covered, file = paste('ExpectedCoverage_Obj_NewModel_Supuesto_280224_', eta[1],'_',eta[2],'.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
...
write.csv(accidents_covered_total, file = paste('ExpectedCoverageTotal_Obj_NewModel_Supuesto_280224_','.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
...
# write.csv(prom_coverage, file = paste('MeanCoverageTotal_Obj_NewModel_Supuesto_280224_','.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
# 
# 
# # MEAN COVERAGE BY SCENARIOS
# pdf(paste("Coverage_NewModel_Supuesto_280224_",len_L[1],'_',eta[1],'_',eta[2],".pdf", sep=""), width = 10)
# par(mfrow = c(1, length(amb)/2), mar=c(4.5, 5, 3.1, 0.9))
# for (a in 1:length(amb[,1])){
#   eta = amb[a,]
#   #pdf(paste("Coverage_NewModel_Supuesto_280224_",eta[1],"_",eta[2],".pdf", sep=""))
#   if(a == 1){
...
# write.csv(prom_coverage_1, file = paste('MeanCoverageTotal_Obj_NewModel_Supuesto_280224_1_','.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
# 
# # MEAN COVERAGE BY DEMAND POINTS
# pdf(paste("Coverage_NewModel_Supuesto_280224_",len_L[1],'_',eta[1],'_',eta[2],"_1.pdf", sep=""), width = 10)
# par(mfrow = c(1, length(amb)/2), mar=c(4.5, 5, 3.1, 0.9))
# for (a in 1:length(amb[,1])){
#   eta = amb[a,]
#   #pdf(paste("Coverage_NewModel_Supuesto_280224_",eta[1],"_",eta[2],".pdf", sep=""))
