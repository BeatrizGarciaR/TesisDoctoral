...
#     eta <- amb[a,]
#     aux_0 <- as.data.frame(read.csv(paste('Tesis_Obj_Zs_270923_',eta[1],'_',eta[2],'.csv', sep="")))
...
#       }
#     }
#     pdf(paste(i,"_runtime_",eta[1],"_",eta[2],".pdf", sep=""))
...
#       }
#     }
#     pdf(paste(i,"_objval_",eta[1],"_",eta[2],".pdf", sep=""))
...
#     points(matrix[16:20], pch=18, cex=1.5, col=6)
#     lines(matrix[16:20], lwd=3, col=6)
#     points(matrix[21:25], pch=19, cex=1.5, col=5)
#     lines(matrix[21:25], lwd=3, col=5)
#     legend(x="topright", legend = len_L, horiz=TRUE, cex = 0.9, fill = c(1, 2, 3, 6, 5), title = "Sites")
#     dev.off()
#   }
#   counti = counti+1
# }
...
#         accidents <- suppressWarnings(as.data.frame(read.table(paste('Accidents_Obj_Zs_270923_', len_I[i], '_', len_L[l], '_', len_S[s], '.txt', sep=""))))
#         print(paste('Accidents_Obj_Zs_270923_', len_I[i], '_', len_L[l], '_', len_S[s], '.txt', sep=""))
#         datos <- c()
#         datos <- cbind(datos, len_I[i], len_L[l], len_S[s])
# 
#         full <- as.data.frame(read.table(paste('Full_Obj_Zs_270923_', len_I[i], '_', len_L[l], '_', len_S[s], '_',eta[1],'_',eta[2],'.txt', sep="")))
...
# 
# 
#         partial1 <- as.data.frame(read.table(paste('Partial1_Obj_Zs_270923_', len_I[i], '_', len_L[l], '_', len_S[s], '_',eta[1],'_',eta[2],'.txt', sep="")))
...
# 
# 
#         partial2 <- as.data.frame(read.table(paste('Partial2_Obj_Zs_270923_', len_I[i], '_', len_L[l], '_', len_S[s], '_',eta[1],'_',eta[2],'.txt', sep="")))
...
# 
# 
#         partial3 <- as.data.frame(read.table(paste('Partial3_Obj_Zs_270923_', len_I[i], '_', len_L[l], '_', len_S[s], '_',eta[1],'_',eta[2],'.txt', sep="")))
...
# 
# 
#         null <- as.data.frame(read.table(paste('Null_Obj_Zs_270923_', len_I[i], '_', len_L[l], '_', len_S[s], '_',eta[1],'_',eta[2],'.txt', sep="")))
...
#   write.csv(accidents_covered, file = paste('ExpectedCoverage_Obj_Zs_270923_', eta[1],'_',eta[2],'.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
...
# }
# write.csv(accidents_covered_total, file = paste('ExpectedCoverageTotal_Obj_Zs_270923_','.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
...
# write.csv(accidents_prom, file = paste('AccidensMean_Obj_Zs_270923.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
# 
# #boxplot(t(full_expected))


#accidents BLS and ALS
# poner aquí la proporción de accidentes por tipo
