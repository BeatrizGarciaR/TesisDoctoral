...
#     eta <- amb[a,]
#     aux_0 <- as.data.frame(read.csv(paste('Tesis_Obj_Zs_261023_',eta[1],'_',eta[2],'.csv', sep="")))
...
for (i in len_I){
  ...
  for (a in 1:length(amb[,1])){
    # run time
    eta <- amb[a,]
    aux_0 <- as.data.frame(read.csv(paste('Tesis_Obj_Zs_261023_',eta[1],'_',eta[2],'.xls', sep="")))
    ...
    legend(x="topright", legend = len_L, horiz=TRUE, cex = 0.9, fill = c(1, 2, 3, 4, 6), title = "Sites")
    dev.off()
  }
  counti = counti+1
}
...
for (a in 1:length(amb[,1])){
  ...
  for (i in 1:length(len_I)){
    for (l in 1:length(len_L)){
      for (s in 1: length(len_S)){
        accidents <- suppressWarnings(as.data.frame(read.table(paste('Accidents_Obj_Zs_261023_', len_I[i], '_', len_L[l], '_', len_S[s], '.txt', sep=""))))
        print(paste('Accidents_Obj_Zs_261023_', len_I[i], '_', len_L[l], '_', len_S[s], '.txt', sep=""))
        datos <- c()
        datos <- cbind(datos, len_I[i], len_L[l], len_S[s])

        full <- as.data.frame(read.table(paste('Full_Obj_Zs_261023_', len_I[i], '_', len_L[l], '_', len_S[s], '_',eta[1],'_',eta[2],'.txt', sep="")))
        ...
        partial1 <- as.data.frame(read.table(paste('Partial1_Obj_Zs_261023_', len_I[i], '_', len_L[l], '_', len_S[s], '_',eta[1],'_',eta[2],'.txt', sep="")))
        ...
        partial2 <- as.data.frame(read.table(paste('Partial2_Obj_Zs_261023_', len_I[i], '_', len_L[l], '_', len_S[s], '_',eta[1],'_',eta[2],'.txt', sep="")))
        ...
        partial3 <- as.data.frame(read.table(paste('Partial3_Obj_Zs_261023_', len_I[i], '_', len_L[l], '_', len_S[s], '_',eta[1],'_',eta[2],'.txt', sep="")))
        ...
        null <- as.data.frame(read.table(paste('Null_Obj_Zs_261023_', len_I[i], '_', len_L[l], '_', len_S[s], '_',eta[1],'_',eta[2],'.txt', sep="")))
  ...
  write.csv(accidents_covered, file = paste('ExpectedCoverage_Obj_Zs_261023_', eta[1],'_',eta[2],'.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
...
write.csv(accidents_covered_total, file = paste('ExpectedCoverageTotal_Obj_Zs_2610923_','.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
...
# write.csv(accidents_prom, file = paste('AccidensMean_Obj_Zs_261023.csv', sep=""), col.names=TRUE, row.names=FALSE, dec = ".")
# 
...
# # DATOS COMPLETOS
# aux <- as.data.frame(read.csv("Tesis_Obj_Zs_261023_20_11.csv"))
# 
# # 168 DEMAND POINTS
