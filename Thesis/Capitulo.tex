...
The Surrogate-based feedback method for the MEC(SABC) model has good results. However, a disadvantage for this method is that we obtain only one solution for the ambulance location from the surrogate model SABC. This is a problem because we disown the optimal solution for the MEC model and we are not sure if the SABC solution is close to that solution.

To improve the solution from SABC model we proposed a local search procedure. For each solution we have from the SABC model, we do exchanges with four different neighborhoods which are taken as follows:
\begin{itemize}
...
\State $x^* = \bar{x}$
\State $t^* = MEC(\bar{x})$
\While{not all neighborhoods have been visited}
	\For{$j \in J$}
		\For{$x' \in N_j$}
    ...
    \EndFor
\EndWhile
\State{ \textbf{return} MEC solution $(x^*,f,g,h,w,z)$ and $t^*$ }
\end{algorithmic}
\end{algorithm}
